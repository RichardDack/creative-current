...
import { auditPageNavigationLinks, fixBrokenNavigationLinks } from './navigationAudit';
...
export function runNavigationTests(): {
  auditResults: ReturnType<typeof auditPageNavigationLinks>;
  fixResults: number;
  summary: string;
} {
  ...
  // Run fixes (dry run)
  const fixResults = fixBrokenNavigationLinks(true);
  console.log(`🔧 ${fixResults} links can be auto-fixed`);
